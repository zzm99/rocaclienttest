...
public class GetInfo : MonoBehaviour
{
    ...
    void Received()
    {
        while (true)
        {
            try
            {
                ...
                var data = JsonConvert.DeserializeObject<InfoJson>(recMes);
                Debug.Log(data.Info.ChapterID);
                Debug.Log(data.Info.LevelIndex);
                Debug.Log(data.Info.Pigment);
            }
            catch (Exception ex) {
                Debug.Log(ex);
            }
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(65+400, 10, 60, 20), "状态信息");
        GUI.Label(new Rect(135+400, 10, 80, 60), staInfo);
        GUI.Label(new Rect(65+400, 150, 80, 20), "接收到消息：");
        GUI.Label(new Rect(155+400, 150, 800, 20), recMes);
        if (GUI.Button(new Rect(65+400, 230, 60, 20), "开始连接"))
        {
            ClickConnect();
        }
        if (GUI.Button(new Rect(65+400, 270, 60, 20), "获取信息"))
        {
            clickSend = true;
        }
    }
