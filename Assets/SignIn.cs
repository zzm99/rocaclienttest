...
public class SignIn : MonoBehaviour
{
    ...
    void SendMessage()
    {
        try
        {
            while (true)
            {
                ...
                {
                    clickSend = false;
                    string msg = "{\"SignIn\": {\"Name\": \"test\", \"PassWord\": \"test\"}}";
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(65+200, 10, 60, 20), "状态信息");
        GUI.Label(new Rect(135+200, 10, 80, 60), staInfo);
        GUI.Label(new Rect(65+200, 150, 80, 20), "接收到消息：");
        GUI.Label(new Rect(155+200, 150, 800, 20), recMes);
        if (GUI.Button(new Rect(65+200, 230, 60, 20), "开始连接"))
        {
            ClickConnect();
        }
        if (GUI.Button(new Rect(65+200, 270, 60, 20), "登录"))
        {
            clickSend = true;
        }
    }
